-- Fuente: https://github.com/pandoc/lua-filters/blob/master/lilypond/lilypond.lua
        ...
        function ()
          pandoc.pipe(
            "lilypond",
            {
              ...
              "-dpixmap-format=pngalpha", -- Enable transparency
              "--output=" .. name, "-"
            },
            input
          )
...
local function process_lilypond(elem, inline)
  ...
  if inline then
    ...
    local image = pandoc.Image(caption, src, title, attrs)
    local content = pandoc.Plain(image)
    return pandoc.Figure(content, {pandoc.Str(caption)}, attrs)
  end
...
-- Main entry point of the filter logic.
-- `elem` is the element being processed and `inline` is a boolean
-- signaling wether the code is inline (true) or block (false)
local function code_transformer(elem, inline)
  if elem.classes:includes("lilypond")
       and not elem.classes:includes("ly-norender") then
    return process_lilypond(elem, inline)
  else
    return elem
  end
end
...
return {
         ...
         {Code      = function(elem) return code_transformer(elem, true) end,
          CodeBlock = function(elem) return code_transformer(elem,false) end},
       }
